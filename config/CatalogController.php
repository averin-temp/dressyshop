...
// die(var_dump($filters));
        // фильтры цвета и размера применяются к запросу продуктов,
        // а не модели, приходится выносить сюда
        foreach ($filters as $filter) {
            if ($filter->type == Filters::TYPE_COLOR) {
                if (isset($options[$filter->id])) {
                    $colors_id = $options[$filter->id];

                    $query->filterWhere(['color_id' => $colors_id]);
                }
            }

            // if ($filter->type == Filters::TYPE_SIZE) {
                // if (isset($options[$filter->id])) {
                    // $sizes = $options[$filter->id];

                    // $query->filterWhere(['size_id' => $sizes]);
                // }
            // }
        }
		//$query->filterWhere(['size_id' => []]);
...
// die();
    ...
    {
		$model_mid = Model::find()->where(['slug'=>'kardigan_cherno_oranzhevogo_cveta'])->one()->id;
